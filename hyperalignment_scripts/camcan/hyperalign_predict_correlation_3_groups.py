...
def calculate_correlation(groupA, groupB, sid, trial):
    ...
    if groupA != groupB:
        hyper_align_lh_procr = np.load(f"{root}/hyper_aligned_new/three_groups/{groupB}_predict_{groupA}/{groupB}_trial_{trial}_predict_{groupA}_{sid}_lh_procr.npy",allow_pickle=True)
        hyper_align_rh_procr = np.load(f"{root}/hyper_aligned_new/three_groups/{groupB}_predict_{groupA}/{groupB}_trial_{trial}_predict_{groupA}_{sid}_rh_procr.npy",allow_pickle=True)
    else:
        hyper_align_lh_procr = np.load(f"{root}/hyper_aligned_new/three_groups/{groupB}/{groupB}_trial_{trial}_{sid}_lh_procr.npy",allow_pickle=True)
        hyper_align_rh_procr = np.load(f"{root}/hyper_aligned_new/three_groups/{groupB}/{groupB}_trial_{trial}_{sid}_rh_procr.npy",allow_pickle=True)
    ...
    for group in ['young','mid','old']:   
        ...
        for trial in range(10):
            ...
            for i in test_list:
                ...
                fn = f'{root}/hyper_aligned_new/three_groups/{group}/{group}_trial_{trial}_{i}_correlation_procr.npy'
                #nb.save(fn, r)
    ...
    for group in ['young','mid','old']:
        for predict_group in ['young','mid','old']:
            ...
            for trial in range(10):
                ...
                for i in test_list:
                    fn = f'{root}/hyper_aligned_new/three_groups/{group}_predict_{predict_group}/{group}_trial_{trial}_predict_{predict_group}_{i}_correlation_procr.npy'
                    job = delayed(nb.record(fn, calculate_correlation))(
